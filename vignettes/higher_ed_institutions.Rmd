...
As of March 13, 2024, the Federal Statistical Office of Germany (Destatis) lists a total of 427 institutions of higher education in the country. The table below lists all these institutions, together with links providing a wide variety of further information for each institution (see documentation below).
...
hochschulen[c(1:max_rows),] |> 
  ...
  dplyr::mutate(
    ...
    STUDIEREN_HS_LABEL = dplyr::if_else(
      is.na(STUDIEREN_ID),
      NA,
      hochschulen[["STUDIEREN_ID"]][1:max_rows]
    ),
...
- the  Research Infrastructure for Research and Innovation Studies (RISIS) ID [see https://register.orgreg.joanneum.at/#/about/1](https://register.orgreg.joanneum.at/#/about/1)). Note that a free account is required to use their services.
...
- the ID of the [World Higher Education Database](https://www.whed.net) by the International Association of Universities.
...
If available, the social media sites are linked:
