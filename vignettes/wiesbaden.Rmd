...
Landesdatenbank NRW website, the code for tables on universities is "213". For
the subset of student statistics, the code is "21311". So let us retrieve all 
table names with code "21311" and turn the resulting data frame into a tibble 
for prettier printing.

```{r df-table}
# Retrieve all tables with code "21311" and turn data frame into tibble
...
# Create named vector of table names

table_names <- df_table[["tablename"]] %>% 
  setNames(
    nm = .
  )
...
# Iterate over every table in the vector of table names, retrieve data with the
# Wiesbaden package,and bind them into one data frame. Note that this may take a 
# while!
df <- purrr::map_dfr(
  table_names,
...
table_21311LS001A <- wiesbaden::retrieve_data(
...
table_21311LS001A_cleaned <- table_21311LS001A %>% 
  clean_df()
  
table_21311LS001A_factor <- table_21311LS001A_cleaned %>% 
  dplyr::mutate(
    dplyr::across(
      where(
        is.character
...
table_21311LS001A <- table_21311LS001A_factor
```
...
We now have a vector of `r length(varibles_unique)` unique variables that are used in the table series
"21311". The data tables use value keys and not the actual value labels (e.g. the
...
str(list_value_labels[2]$DLAND)
str(list_value_labels[3]$BILHS1)
str(list_value_labels[4]$BILSF1)
...
# We use purrr::discard to drop all these elements of type "character".
...
square brackets like this `list_value_labels[[1]]`.
...
in this vignette was obtained from the Landesdatenbank NRW using the code 
